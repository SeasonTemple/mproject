    ...
    <div class="left" :class="{isSwitch: mode=='register'}">
      <el-form ref="form" :model="form">
        ...
        <el-form-item label="Password" :class="{isFocus: pwdFocus.isFocus}">
          <el-input v-model="form.password" autocomplete show-password @focus.stop="inputFocus(pwdFocus.name)"
            @blur.stop="inputblur(pwdFocus.name)" prop="password">
            <i class="el-icon-lock" slot="prepend"></i>
          </el-input>
        </el-form-item>
        <el-form-item :class="{}" v-if="mode=='register'">
          <el-input v-model="form.valiCode" prop="valiCode" id="valiCode">
